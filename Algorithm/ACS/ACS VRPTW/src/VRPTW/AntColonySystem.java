...
public class AntColonySystem {
	...
	public void init() {
		...
		System.out.println(pheromone_0);
	...
	public void construct_solution() {
		...
		for (int i = 0; i < agentNr; i++) {
			...
			// untreated[i]: 记录每一位agent k未服务过的客户，初始化：1-100
			//
	...
	public void update_pheromone() {
		...
		for (int i = 0; i < now_best.routes.size(); i++){
			...
			for ( int k = 0; k < now_best.routes.get(i).customers.size() - 1; ++k )
				System.out.print( now_best.routes.get(i).customers.get(k) + " -> ");
			System.out.println( now_best.routes.get(i).customers.get(now_best.routes.get(i).customers.size() - 1));
		}
	...
	public Solution ACS_Strategy(){
		...
		for (int i = 0; i < IterMax; i++) {
			System.out.println("*************************** No." + i + " iterations *********************************");
			reset();//初始化agent信息 
