...
public class Instance {
	...
	public void populateFromHombergFile(String filename) {
		try {
			// parameters.getCurrDir() return packageName+className
			// output: C:\Users\zichengk\Documents\毕业设计\omatest-master\src\com
...
//			System.out.println(depot.getNumber());  // 0
			depot.setXCoordinate(in.nextDouble());
			depot.setYCoordinate(in.nextDouble());
//			System.out.println(depot.getXCoordinate());
//			System.out.println(depot.getYCoordinate());
			in.nextDouble();
			depot.setStartTW(in.nextInt());
			depot.setEndTW(in.nextInt());
			in.nextDouble();
			// this data is used for DrawPanel
			maxX = Math.max(depot.getXCoordinate(), maxX);
			maxY = Math.max(depot.getYCoordinate(), maxY);
			minX = Math.min(depot.getXCoordinate(), minX);
			minY = Math.min(depot.getYCoordinate(), minY);
			depots.add(depot);
			...
			{					
				...
				// this data is used for DrawPanel
				maxX = Math.max(customer.getXCoordinate(), maxX);
				maxY = Math.max(customer.getYCoordinate(), maxY);
				minX = Math.min(customer.getXCoordinate(), minX);
				minY = Math.min(customer.getYCoordinate(), minY);

				// add customer to customers list
				customers.add(customer);
				customersNr++;
			}// end for customers
//			System.out.println("CustomersNr"+customersNr);
			in.close();
	...
	/**
	 *  Assign to each customer the closest depot based on distances
	 */
