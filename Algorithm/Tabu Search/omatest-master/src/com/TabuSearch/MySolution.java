...
public class MySolution extends SolutionAdapter{
	...
	public void initializeRoutes(Instance instance) {
		...
		System.out.println("Number of Depot and Vehicles: " + instance.getDepotsNr() + " " + instance.getVehiclesNr());
		...
		for (int i = 0; i < instance.getDepotsNr(); ++i)
			for (int j = 0; j < instance.getVehiclesNr(); ++j){
					...
					Cost cost = new Cost();
					routes[i][j].setCost(cost);  // all zero
					
					// assign vehicle
					Vehicle vehicle = new Vehicle();
					vehicle.setCapacity(instance.getCapacity(i, 0));  // capacity[0][0]: 200
//					System.out.println("Vehicles' capacity is (in MySolution.java)" + vehicle.getCapacity());
	...
	public void buildInitialRoutes1(Instance instance) {
		...
		for( int i = 0; i < instance.getDepotsNr(); ++i) { // instance.getDepotsNr(): 1
			debug.append("\n");
			assignedCustomersNr = instance.getDepot(i).getAssignedCustomersNr(); // assignedCustomersNr: 100
			...
			System.out.println(startCustomer + " " + assignedCustomersNr);
...
//					if (k == 0){
//						System.out.println(customerChosenPtr.getCapacity() + " " + route.getCost().load + " " +
//						route.getLoadAdmited());
//					}
...
//			for (int ii = 0; ii < instance.getVehiclesNr() - 1; ++ii){
//				System.out.println("-------- Route " + ii + "-------------");
//				for(int kk = 0; kk < routes[0][ii].getCustomersLength(); kk++) {
//					System.out.print(routes[0][ii].getCustomer(kk).getNumber() + " ");
//				}
//				System.out.println();
//			}
	 ...
	 * @param instance
	 * @param route
	 * @param customerChosenPtr
	 *
	 * 类似于链表的插入操作，插入的位置取决于EndTw和Cost。
	 */
...
//		System.out.print(customerChosenPtr.getNumber() + " ");
